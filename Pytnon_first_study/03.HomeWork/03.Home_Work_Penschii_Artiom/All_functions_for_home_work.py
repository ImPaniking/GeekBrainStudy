...
def input_number_test_biger_then_zero(text : str) -> int:
    ...
    while int_test:
        ...
        if number[0] == "-": # проверка на <0 - если в начале есть символ "-" - сохраняет его в булевую переменную ( в данном случае не используется )
            is_minus = True
            number = number.replace("-","") # в данном случае - просто убирает 0 и сообщает что число стало положительным 
...
def array_creation(length_of_array : int , min_for_random : int , max_for_random : int) -> list: # Создание списка из случайных чисел
...
def array_creation_float(length_of_array : int,bit_depth = 10 , round_param = 3) -> list:
    ...
    list_of_numbers = []
    for i in range(0,length_of_array):
        list_of_numbers.append(round(random.random()*bit_depth,round_param)) # тут я понимаю что нужно проработать на сколько умножать - планирую продумать это в другой функции
    return list_of_numbers
...
def in_byte_transfer(number : int) -> list:
    ...
    while ostatok >= 1:
        ...
        number = ostatok
    sort_array(list_of_bites) # разворачивает список ( кстати - пример Void функции - если я правильно понмю , в C# списки и массивы были ссылочными типами, по этому любое изменение - меняет его везед)
