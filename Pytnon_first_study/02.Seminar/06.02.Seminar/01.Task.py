...
def cal_with_stoto(list_of_math: list) -> list:
    ...
    while "(" in list_of_math:
        ...
        for i,item in enumerate(list_of_math):
            if item == "(":
                max_index = i
        for i in list_of_math[max_index+1:list_of_math.index(")")]:
            new_list.append(i)
        print(new_list)
        # if "(" in new_list:
        #     cal_with_stoto(new_list)
        # else:
        for i in range(len(list_of_math[max_index:list_of_math.index(")")+1])):
            del list_of_math[max_index]
        list_of_math.insert(max_index,calt_trad(new_list)[0])
    else:
